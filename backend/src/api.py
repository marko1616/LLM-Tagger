...
async def list_datasets() -> JSONResponse:
    ...
    """
    return JSONResponse({"message": "Datasets listed", "datasets": db.list_datasets()})
...
async def create_dataset(dataset: Dataset) -> JSONResponse:
    ...
    if dataset.name == "":
        return JSONResponse(
            {"message": "Dataset name cannot be empty"}, status_code=400
        )
...
async def create_dataset_item(dataset_name: str, item: DatasetItem) -> JSONResponse:
    """
    Creates a dataset item.
    """
    if item.name == "":
        return JSONResponse(
            {"message": "Dataset item name cannot be empty"}, status_code=400
        )
...
async def get_dataset_item(dataset_name: str, item_name: str) -> JSONResponse:
    ...
    for item in dataset.items:
        if item.name == item_name:
            return JSONResponse(
                {"message": "Dataset item retrieved", "item": item.model_dump()}
            )
...
async def list_plugins():
    ...
    for interface in plugin_interfaces:
        plugin_info.append(
            {
                "url": f"/plugins/{interface.api_name}",
